...
router.patch('/:id', requireAuth, async (req, res) => {
  ...
  try {
    ...
    res.json({ inventory: { id: req.params.id, name: name.trim() } })
  } catch (err) {
    res.status(500).json({ error: 'Server error' })
  }
})
...
router.delete('/:id', requireAuth, async (req, res) => {
  try {
    ...
    await db.execute({ sql: 'DELETE FROM inventories WHERE id = ?', args: [req.params.id] })
    res.status(204).end()
  } catch (err) {
    res.status(500).json({ error: 'Server error' })
  }
  ...
  try {
    ...
    await db.execute({
      sql: `INSERT INTO invite_tokens (token, inventory_id, email, role, invited_by, expires_at)
            VALUES (?, ?, ?, ?, ?, ?)`,
      args: [token, req.params.id, email.toLowerCase(), role, req.user.id, expires],
    })
...
router.post('/:id/categories', requireAuth, async (req, res) => {
  ...
  try {
    ...
    await db.execute({
      ...
      args: [id, req.params.id, name.trim()],
    })
    res.status(201).json({ category: { id, inventory_id: req.params.id, name: name.trim() } })
  } catch (err) {
    res.status(500).json({ error: 'Server error' })
  }
})
...
router.post('/:id/locations', requireAuth, async (req, res) => {
  ...
  try {
    ...
    await db.execute({
      ...
      args: [id, req.params.id, name.trim(), parent_id || null],
    })
    res.status(201).json({ location: { id, inventory_id: req.params.id, name: name.trim(), parent_id: parent_id || null } })
  } catch (err) {
    res.status(500).json({ error: 'Server error' })
  }
})
